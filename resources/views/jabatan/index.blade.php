                    ...
                    <table class="min-w-full text-sm bg-white divide-y-2 divide-gray-200">
                        ...
                        <tbody class="divide-y divide-gray-200">
                            @forelse ($jabatans as $jabatan)
                                <tr>
                                    <td class="px-4 py-2 font-medium text-gray-900 whitespace-nowrap">{{ $jabatan->nama_jabatan }}</td>
                                    <td class="flex justify-end gap-2 px-4 py-2 whitespace-nowrap">
                                        <a href="{{ route('jabatan.edit', $jabatan) }}" class="inline-block px-4 py-2 text-xs font-medium text-white bg-indigo-600 rounded hover:bg-indigo-700">Edit</a>
                                        <form method="POST" action="{{ route('jabatan.destroy', $jabatan) }}" onsubmit="return confirm('Yakin hapus?');">
                                            @csrf
                                            @method('DELETE')
